...
const adminNavItems = [
  ...
  { label: "Attendance Overview", href: "/admin/attendance-overview", icon: Users },
...
export default async function AdminLayout({
  ...
  return (
    <div className="min-h-screen flex flex-col bg-bg text-text">
      {/* Admin Top Header */}
      <header className="sticky top-0 z-40 bg-bg-elevated border-b border-border text-text px-4 py-3 sm:px-6 shadow-md">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-500 dark:text-amber-400 border border-amber-500/30">
                  Single Owner
                </span>
              </div>
          ...
          <Link
            href="/home"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-bg-subtle hover:bg-bg-elevated text-text border border-border transition"
          >
      ...
      <nav className="bg-bg-elevated border-b border-border px-4 sm:px-6 overflow-x-auto scrollbar-none">
          ...
          {adminNavItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold text-text-muted hover:bg-bg-subtle hover:text-text transition whitespace-nowrap"
              >
