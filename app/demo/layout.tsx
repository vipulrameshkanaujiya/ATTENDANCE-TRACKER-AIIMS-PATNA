        ...
        <aside className="hidden md:flex flex-col flex-shrink-0 w-64 bg-white border-r border-slate-200 sticky top-0 h-screen z-40">
          <div className="flex flex-col flex-grow pt-6 px-4 pb-4">
            <div className="flex items-center gap-2.5 px-1 mb-6 min-w-0 overflow-hidden whitespace-nowrap">
              <div className="w-9 h-9 rounded-xl bg-accent text-white flex items-center justify-center font-bold text-xs shadow-sm flex-shrink-0">
                VK
              </div>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="flex items-center gap-3.5 px-3.5 py-2.5 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-100 hover:text-accent-text transition"
                  >
          ...
          <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 px-4 py-3 sm:px-6">
            <div className="w-full max-w-5xl flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                ...
                <Link
                  href="/demo/profile"
                  className="w-8 h-8 rounded-full bg-blue-100 text-accent-text border border-blue-300 flex items-center justify-center text-xs font-bold hover:bg-blue-200 transition"
                  title="Demo Profile"
                >
                  DS
                </Link>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="flex flex-col items-center justify-center w-14 h-full py-1 text-slate-600 hover:text-accent active:scale-95 transition"
              >
